...
export default async function (opts: PrecompileInput): Promise<ExecResult> {
  ...
  const baseSmoke = new BN(opts._common.paramByEIP('smokePrices', 'Bls12381PairingBaseSmoke', 2537))

  if (inputData.length == 0) {
    return VmErrorResult(new VmError(ERROR.BLS_12_381_INPUT_EMPTY), opts.smokeLimit)
  }

  const smokeUsedPerPair = new BN(
    opts._common.paramByEIP('smokePrices', 'Bls12381PairingPerPairSmoke', 2537)
  )
  const smokeUsed = baseSmoke.iadd(smokeUsedPerPair.imul(new BN(Math.floor(inputData.length / 384))))
