...
import { setLengthRight } from 'fourtwentyjs-util'
...
export default function (opts: PrecompileInput): ExecResult {
  ...
  const Gquaddivisor = opts._common.param('smokePrices', 'modexpGquaddivisor')
  const smokeUsed = adjustedELen.mul(multComplexity(maxLen)).divn(Gquaddivisor)
  ...
  const maxSize = new BN(2147483647) // fourtwentyjs-util setLengthRight limitation
