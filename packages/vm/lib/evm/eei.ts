import { Account, Address, BN } from 'fourtwentyjs-util'
 ...
 * the ewasm EEI [spec](https://github.com/ewasm/design/blob/master/fourtwenty_interface.md).
...
export default class EEI {
  ...
  constructor(env: Env, state: StateManager, evm: EVM, common: Common, smokeLeft: BN) {
    this._env = env
    this._state = state
    this._evm = evm
    this._lastReturned = Buffer.alloc(0)
    this._common = common
    this._smokeLeft = smokeLeft
    this._result = {
      logs: [],
  ...
  /**
   * Subtracts an amount from the smoke counter.
   * @param amount - Amount of smoke to consume
   * @throws if out of smoke
   */
  useSmoke(amount: BN): void {
    this._smokeLeft.isub(amount)
    if (this._smokeLeft.ltn(0)) {
      this._smokeLeft = new BN(0)
      trap(ERROR.OUT_OF_GAS)
    }
  }

  /**
   * Adds a positive amount to the smoke counter.
   * @param amount - Amount of smoke refunded
   */
  refundSmoke(amount: BN): void {
    this._evm._refund.iadd(amount)
  }

  /**
   * Reduces amount of smoke to be refunded by a positive value.
   * @param amount - Amount to subtract from smoke refunds
   */
  ...
  /**
   * Returns price of smoke in current environment.
   */
  getTxSmokePrice(): BN {
    return this._env.smokePrice
  }
  ...
  /**
   * Returns the block's smoke limit.
   */
  getBlockSmokeLimit(): BN {
    return this._env.block.header.smokeLimit
  }
  ...
  /**
   * Returns the current smokeCounter.
   */
  getSmokeLeft(): BN {
    return this._smokeLeft.clone()
  }
  ...
  async _selfDestruct(toAddress: Address): Promise<void> {
    ...
    if (!this._result.selfdestruct[this._env.address.buf.toString('hex')]) {
      this.refundSmoke(new BN(this._common.param('smokePrices', 'selfdestructRefund')))
    }
   ...
   */
  async call(smokeLimit: BN, address: Address, value: BN, data: Buffer): Promise<BN> {
   ...
   */
  async callCode(smokeLimit: BN, address: Address, value: BN, data: Buffer): Promise<BN> {
    const msg = new Message({
      caller: this._env.address,
      smokeLimit,
   ...
   * a non-zero value.
   */
  async callStatic(smokeLimit: BN, address: Address, value: BN, data: Buffer): Promise<BN> {
    const msg = new Message({
      caller: this._env.address,
      smokeLimit,
      to: address,
   ...
   */
  async callDelegate(smokeLimit: BN, address: Address, value: BN, data: Buffer): Promise<BN> {
    const msg = new Message({
  ...
  async _baseCall(msg: Message): Promise<BN> {
    ...
    this.useSmoke(results.smokeUsed)
   ...
   */
  async create(smokeLimit: BN, value: BN, data: Buffer, salt: Buffer | null = null): Promise<BN> {
    const selfdestruct = { ...this._result.selfdestruct }
    const msg = new Message({
      caller: this._env.address,
      smokeLimit,
   ...
   */
  async create2(smokeLimit: BN, value: BN, data: Buffer, salt: Buffer): Promise<BN> {
    return this.create(smokeLimit, value, data, salt)
  }
  ...
  private _getReturnCode(results: EVMResult) {
    ...
    // https://github.com/ewasm/design/blob/38eeded28765f3e193e12881ea72a6ab807a3371/fourtwenty_interface.md
    if (results.execResult.exceptionError) {
      return new BN(0)
    } else {
      return new BN(1)
