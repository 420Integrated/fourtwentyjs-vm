...
// TODO: All smoke price changes should be moved to common
 ...
 *
 * @param common {Common} Fourtwentyjs Common metadata object.
...
export function getOpcodesForHF(common: Common): OpcodeList {
  ...
  for (const key in opcodeBuilder) {
    const baseFee = common.param('smokePrices', opcodeBuilder[key].name.toLowerCase())
    // explicitly verify that we have defined a base fee
    if (baseFee === undefined) {
      throw new Error(`base fee not defined for: ${opcodeBuilder[key].name}`)
    }
    opcodeBuilder[key].fee = common.param('smokePrices', opcodeBuilder[key].name.toLowerCase())
  }
