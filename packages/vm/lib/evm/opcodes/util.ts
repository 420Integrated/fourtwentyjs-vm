import { Address, BN, keccak256, setLengthRight, setLengthLeft } from 'fourtwentyjs-util'
 ...
 *
 * the data with zeros to `length`.
 * @param {BN} smokeLimit - requested smoke Limit
 * @param {BN} smokeLeft - current smoke left
 * @param {RunState} runState - the current runState
 */
export function maxCallSmoke(smokeLimit: BN, smokeLeft: BN, runState: RunState): BN {
  const isTangerineWhistleOrLater = runState._common.gteHardfork('tangerineWhistle')
  if (isTangerineWhistleOrLater) {
    const smokeAllowed = smokeLeft.sub(smokeLeft.divn(64))
    return smokeLimit.gt(smokeAllowed) ? smokeAllowed : smokeLimit
  } else {
    return smokeLimit
  }
}
...
export function subMemUsage(runState: RunState, offset: BN, length: BN) {
  ...
  const fee = new BN(runState._common.param('smokePrices', 'memory'))
  const quadCoeff = new BN(runState._common.param('smokePrices', 'quadCoeffDiv'))
  // words * 3 + words ^2 / 512
  const cost = words.mul(fee).add(words.mul(words).div(quadCoeff))

  if (cost.gt(runState.highestMemCost)) {
    runState.eei.useSmoke(cost.sub(runState.highestMemCost))
