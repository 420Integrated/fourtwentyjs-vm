  ...
  if (!runState.accessedAddresses.has(addressStr)) {
    ...
    if (baseFee !== undefined) {
      runState.eei.useSmoke(
        new BN(runState._common.param('smokePrices', 'coldaccountaccess') - baseFee)
      )
    }
    // Warm: (selfdestruct beneficiary address reads are not charged when warm)
  } else if (baseFee !== undefined && baseFee > 0) {
    runState.eei.useSmoke(new BN(runState._common.param('smokePrices', 'warmstorageread') - baseFee))
  }
}
...
export function accessStorageEIP2929(runState: RunState, key: Buffer, isSstore: boolean) {
  ...
  const baseFee = !isSstore ? runState._common.param('smokePrices', 'sload') : 0
  ...
  if (!keysAtAddress) {
    ...
    runState.eei.useSmoke(new BN(runState._common.param('smokePrices', 'coldsload') - baseFee))
 ...
 */
export function adjustSstoreSmokeEIP2929(
  runState: RunState,
  key: Buffer,
  defaultCost: number,
  costName: string
  ...
  const keyStr = key.toString('hex')
  const address = runState.eei.getAddress().toString()
  const warmRead = runState._common.param('smokePrices', 'warmstorageread')
  const coldSload = runState._common.param('smokePrices', 'coldsload')
  ...
  if (runState.accessedStorage.has(address) && runState.accessedStorage.get(address).has(keyStr)) {
    switch (costName) {
      ...
      case 'noop':
        return warmRead
      case 'initRefund':
        return runState._common.param('smokePrices', 'sstoreInitSmokeEIP2200') - warmRead
      case 'cleanRefund':
        return runState._common.param('smokePrices', 'sstoreReset') - coldSload - warmRead
    }
  }
