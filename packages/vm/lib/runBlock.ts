...
export default async function runBlock(this: VM, opts: RunBlockOpts): Promise<RunBlockResult> {
  ...
  if (generateStateRoot) {
    ...
    if (!result.smokeUsed.eq(block.header.smokeUsed)) {
      throw new Error('invalid smokeUsed')
    }
...
async function applyBlock(this: VM, block: Block, opts: RunBlockOpts) {
  ...
  if (!opts.skipBlockValidation) {
    if (block.header.smokeLimit.gte(new BN('8000000000000000', 16))) {
      throw new Error('Invalid block with smoke limit greater than (2^63 - 1)')
    } else {
 ...
 * as well as smoke usage and some relevant data. This method is
...
async function applyTransactions(this: VM, block: Block, opts: RunBlockOpts) {
  ...
  // the total amount of smoke used processing these transactions
  let smokeUsed = new BN(0)
  ...
  for (let txIdx = 0; txIdx < block.transactions.length; txIdx++) {
    ...
    const smokeLimitIsHigherThanBlock = block.header.smokeLimit.lt(tx.smokeLimit.add(smokeUsed))
    if (smokeLimitIsHigherThanBlock) {
    ...
    // Add to total block smoke usage
    smokeUsed = smokeUsed.add(txRes.smokeUsed)
    // Combine blooms via bitwise OR
    bloom.or(txRes.bloom)

    const abstractTxReceipt: TxReceipt = {
      smokeUsed: smokeUsed.toArrayLike(Buffer),
