...
import { Account, Address } from 'fourtwentyjs-util'
...
export default class VM extends AsyncEventEmitter {
  ...
  async init(): Promise<void> {
    ...
    if (this._opts.activatePrecompiles && !this._opts.stateManager) {
      ...
      // put 1 wei in each of the precompiles in order to make the accounts non-empty and thus not have them deduct `callNewAccount` smoke.
      await Promise.all(
   ...
   *
   * @param blockchain -  An [@fourtwentyjs/blockchain](https://github.com/420integrated/fourtwentyjs-vm/tree/master/packages/blockchain) object to process
   */
