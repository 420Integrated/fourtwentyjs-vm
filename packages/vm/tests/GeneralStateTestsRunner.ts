...
import { Account, BN, toBuffer } from 'fourtwentyjs-util'
...
function parseTestCases(
  ...
  if (testData['post'][forkConfigTestSuite]) {
    testCases = testData['post'][forkConfigTestSuite].map((testCase: any) => {
      ...
      if (smokeLimit !== undefined && testIndexes['smoke'] !== smokeLimit) {
        return null
      }

      tx.data = testData.transaction.data[testIndexes['data']]
      tx.smokeLimit = testData.transaction.smokeLimit[testIndexes['smoke']]
  ...
  if (options.jsontrace) {
    vm.on('step', function (e: InterpreterStep) {
      ...
      const opTrace = {
        pc: e.pc,
        op: e.opcode.name,
        smoke: '0x' + e.smokeLeft.toString('hex'),
        smokeCost: '0x' + e.opcode.fee.toString(16),
        stack: hexStack,
        depth: e.depth,
