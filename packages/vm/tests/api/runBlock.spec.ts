...
import { Address, BN, rlp } from 'fourtwentyjs-util'
...
tape('should fail when block smoke limit higher than 2^63-1', async (t) => {
  const suite = setup()

  const block = Block.fromBlockData({
    header: {
      ...suite.data.blocks[0].header,
      smokeLimit: Buffer.from('8000000000000000', 16),
    },
  })
  await suite.p
    .runBlock({ block })
...
tape('should fail when tx smoke limit higher than block smoke limit', async (t) => {
  const suite = setup()

  const blockRlp = suite.data.blocks[0].rlp
  const block = Object.create(Block.fromRLPSerializedBlock(blockRlp))
  // modify first tx's smokeLimit
  const { nonce, smokePrice, to, value, data, v, r, s } = block.transactions[0]

  const smokeLimit = new BN(Buffer.from('3fefba', 'hex'))
  const opts = { common: block._common }
  block.transactions[0] = new Transaction(nonce, smokePrice, smokeLimit, to, value, data, v, r, s, opts)

  await suite.p
    .runBlock({ block, skipBlockValidation: true })
    .then(() => t.fail('should have returned error'))
    .catch((e) => t.ok(e.message.includes('higher smoke limit')))
...
tape('should run valid block', async (t) => {
  ...
  t.equal(
    res.results[0].smokeUsed.toString('hex'),
    '5208',
    'actual smoke used should equal blockHeader smokeUsed'
  )
