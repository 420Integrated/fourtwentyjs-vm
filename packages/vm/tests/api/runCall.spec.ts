...
import { Address, BN, keccak256, padToEven } from 'fourtwentyjs-util'
...
tape('Constantinople: EIP-1014 CREATE2 creates the right contract address', async (t) => {
  ...
  for (let value = 0; value <= 1000; value += 20) {
    ...
    const runCallArgs = {
      ...
      smokeLimit: new BN(0xffffffffff), // ensure we pass a lot of smoke, so we do not run out of smoke
...
tape('Ensure that precompile activation creates non-empty accounts', async (t) => {
  ...
  /*
      idea: call the Identity precompile with nonzero value in order to trigger "callNewAccount" for the non-activated VM and do not deduct this
            when calling from the activated VM. Explicitly check that the difference in smoke cost is equal to the common callNewAccount smoke.
      code:             remarks: (top of the stack is at the zero index)       
        PUSH1 0x00
        DUP1
        DUP1
  ...
  const diff = resultNotActivated.smokeUsed.sub(resultActivated.smokeUsed)
  const expected = common.param('smokePrices', 'callNewAccount')
...
tape('Ensure that Istanbul sstoreCleanRefundEIP2200 smoke is applied correctly', async (t) => {
  // setup the accounts for this test
  const caller = new Address(Buffer.from('00000000000000000000000000000000000000ee', 'hex')) // caller addres
  ...
  console.log(result.smokeUsed, result.execResult.smokeRefund)
  t.equal(result.smokeUsed.toNumber(), 5812, 'smoke used incorrect')
  t.equal(result.execResult.smokeRefund!.toNumber(), 4200, 'smoke refund incorrect')
