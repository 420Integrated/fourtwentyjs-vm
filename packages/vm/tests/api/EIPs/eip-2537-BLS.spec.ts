...
tape('EIP-2537 BLS tests', (t) => {
  t.test('BLS precompiles should not be available if EIP not activated', async (st) => {
    ...
    for (const address of precompiles) {
      ...
      if (!result.execResult.smokeUsed.eq(new BN(0))) {
        st.fail('BLS precompiles should not use any smoke if EIP not activated')
      }
  ...
  t.test('BLS precompiles should throw on empty inputs', async (st) => {
    ...
    for (const address of precompiles) {
      ...
      if (!result.execResult.smokeUsed.eq(new BN(0xffffffffff))) {
        st.fail('BLS precompiles should use all smoke on empty inputs')
      }
