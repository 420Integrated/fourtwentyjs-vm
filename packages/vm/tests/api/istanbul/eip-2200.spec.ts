...
const testCases = [
  ...
  { original: new BN(1), smoke: new BN(2307), code: '6001600055', used: 806, refund: 0 }, // 1 -> 1 (2301 sentry + 2xPUSH)
]
...
tape('Istanbul: EIP-2200', async (t) => {
  t.test('net-metering SSTORE', async (st) => {
    ...
    for (const testCase of testCases) {
      ...
      const runCallArgs = {
        caller,
        smokeLimit: testCase.smoke ? testCase.smoke : new BN(0xffffffffff),
        to: addr,
      }
      ...
      try {
        ...
        st.assert(new BN(testCase.used).eq(res.smokeUsed))
        st.assert(new BN(testCase.refund).eq(res.execResult.smokeRefund!))
      } catch (e) {
        st.fail(e.message)
      }
    }
