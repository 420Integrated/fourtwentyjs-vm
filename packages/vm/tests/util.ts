...
import { Transaction, TxOptions } from '@fourtwentyjs/tx'
import { Block, BlockHeader, BlockOptions } from '@fourtwentyjs/block'
 ...
 * @param {TxOptions} opts Tx opts that can include an @fourtwentyjs/common object
...
/**
 * verifySmoke by computing the difference of coinbase account balance
 * @param {Object} results  to verify
 * @param {Object} testData from tests repo
 */
export function verifySmoke(results: any, testData: any, t: tape.Test) {
  ...
  if (!balance.isZero()) {
    const amountSpent = results.smokeUsed.mul(testData.transaction.smokePrice)
    t.ok(amountSpent.eq(balance), 'correct smoke')
  } else {
