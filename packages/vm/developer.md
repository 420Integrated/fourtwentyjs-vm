...
Tests can be found in the `tests` directory. There are test runners for [State tests](http://www.ethdocs.org/en/latest/contracts-and-transactions/ethereum-tests/state_tests/index.html) and [Blockchain tests](http://www.ethdocs.org/en/latest/contracts-and-transactions/ethereum-tests/blockchain_tests/index.html). VM tests are disabled since Frontier smoke costs are not supported any more. Tests are then executed by the [fourtwentyjs-testing](https://github.com/420integrated/fourtwentyjs-testing) utility library using the official client-independent [420coin tests](https://github.com/420integrated/go-420coin/tests).

For a wider picture about how to use tests to implement EIPs you can have a look at this [Reddit post](https://www.reddit.com/r/ethereum/comments/6kc5g3/fourtwentyjs_team_is_seeking_contributors/)
or the associated YouTube video introduction to [Core Development with Fourtwentyjs-vm](https://www.youtube.com/watch?v=L0BVDl6HZzk).
...
`ts-node ./tests/tester --blockchain --dir='bcBlockSmokeLimitTest'`

Running a specific state test case:

`ts-node ./tests/tester --state --test='stackOverflow'`

Only run test cases with selected `data`, `smoke` and/or `value` values (see
...
`ts-node ./tests/tester --state --test='CreateCollisionToEmpty' --data=0 --smoke=1 --value=0`
...
`npm run formatTest -- -t "./tests/tester --blockchain --dir='bcBlockSmokeLimitTest'"`
...
Tests and checks are run in CI using [Github Actions](https://github.com/420integrated/fourtwentyjs-vm/actions). The configuration can be found in `.github/workflows`.
...
All/most State tests are replicated as Blockchain tests in a `GeneralStateTests` [sub directory](https://github.com/420integrated/go-420coin/tests/tree/develop/BlockchainTests/GeneralStateTests) in the 420coin tests repo, so for debugging single test cases the Blockchain test version of the State test can be used.

#### Comparing Stack Traces

Other client implementations often also provide functionality for output trace information.

A convenient way is to use a local `g420` installation (can be the binary installation and doesn't has to be build from source or something) and then use the included `evm` tool like:

```shell
evm --json --nomemory statetest node_modules/fourtwentyjs-testing/tests/GeneralStateTests/stCreate2/create2collisionCode2.json
```
...
View the historical benchmark data for the master branch on the [github page](http://fourtwentyjs.github.io/fourtwentyjs-vm/dev/bench/).
