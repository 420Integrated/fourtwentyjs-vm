...
Implements 420coin's VM in Javascript.
...
An 420coin test suite compliant `MuirGlacier` HF implementation is available
...
An 420coin test suite compliant `Istanbul` HF implementation is available
...
vm.runCode({
  ...
  .then((results) => {
    ...
    console.log(`smokeUsed : ${results.smokeUsed.toString()}`)
  })
  .catch(console.error)
```
...
To build the VM for standalone use in the browser, see: [Running the VM in a browser](https://github.com/420integrated/fourtwentyjs-vm/tree/master/examples/run-code-browser).
...
The API for the `StateManager` is currently in `Beta`, separate documentation can be found [here](./docs/classes/statemanager.md), see also [release notes](https://github.com/420integrated/fourtwentyjs-vm/releases/tag/v2.5.0) from the `v2.5.0` VM release for details on the `StateManager` rewrite.
...
[vm-actions-badge]: https://github.com/420integrated/fourtwentyjs-vm/workflows/VM%20Test/badge.svg
[vm-actions-link]: https://github.com/420integrated/fourtwentyjs-vm/actions?query=workflow%3A%22VM+Test%22
