...
tape('blockchain test', (t) => {
  ...
  t.test('should add 10 blocks, one at a time', async (st) => {
    ...
    const blocks: Block[] = []
    const smokeLimit = 8000000

    const genesis = Block.genesis({ header: { smokeLimit } })
    blocks.push(genesis)
    await blockchain.putGenesis(genesis)
  ...
  t.test('should put multiple blocks at once', async (st) => {
    ...
    const blocks: Block[] = []
    const genesis = Block.genesis({ header: { smokeLimit: 8000000 } })
  ...
  t.test('should get latest', async (st) => {
    ...
    const genesis = Block.genesis({ header: { smokeLimit } }, opts)
    await blockchain.putGenesis(genesis)
  ...
  t.test('mismatched chains', async (st) => {
    ...
    const genesis = Block.genesis({ header: { smokeLimit } }, { common })
