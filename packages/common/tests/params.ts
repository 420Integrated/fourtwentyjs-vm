...
tape('[Common]: Parameter access for param(), paramByHardfork()', function (t: tape.Test) {
  t.test('Basic usage', function (st: tape.Test) {
    ...
    st.equal(c.paramByHardfork('smokePrices', 'ecAdd', 'byzantium'), 500, msg)

    msg = 'Should return correct value for HF set in class'
    c.setHardfork('byzantium')
    st.equal(c.param('smokePrices', 'ecAdd'), 500, msg)
    c.setHardfork('istanbul')
    st.equal(c.param('smokePrices', 'ecAdd'), 150, msg)
    c.setHardfork('muirGlacier')
    st.equal(c.param('smokePrices', 'ecAdd'), 150, msg)
    c.setHardfork('berlin')
    st.equal(c.param('smokePrices', 'beginsub'), 2, msg)

    msg = 'Should return null for non-existing value'
    st.equal(c.param('smokePrices', 'notexistingvalue'), null, msg)
    st.equal(c.paramByHardfork('smokePrices', 'notexistingvalue', 'byzantium'), null, msg)
    ...
    st.equal(c.param('smokePrices', 'ecAdd'), 12345, msg)
    */
  ...
  t.test('Error cases for param(), paramByHardfork()', function (st: tape.Test) {
    ...
    let f = function () {
      c.paramByHardfork('smokePrizes', 'ecAdd', 'byzantium')
    }
    let msg = 'Should throw when called with non-existing topic'
    st.throws(f, /Topic smokePrizes not defined$/, msg)

    c.setHardfork('byzantium')
    st.equal(c.param('smokePrices', 'ecAdd'), 500, 'Should return correct value for HF set in class')
  ...
  t.test('Parameter updates', function (st: tape.Test) {
    ...
    msg = 'Should return updated sstore smoke prices for constantinople'
    st.equal(c.paramByHardfork('smokePrices', 'netSstoreNoopSmoke', 'constantinople'), 200, msg)

    msg = 'Should nullify SSTORE related values for petersburg'
    st.equal(c.paramByHardfork('smokePrices', 'netSstoreNoopSmoke', 'petersburg'), null, msg)
  ...
  t.test('EIP param access, paramByEIP()', function (st: tape.Test) {
    ...
    st.equal(c.paramByEIP('smokePrices', 'notexistingvalue', 2537), null, msg)

    const UNSUPPORTED_EIP = 1000000
    let f = function () {
      c.paramByEIP('smokePrices', 'Bls12381G1AddSmoke', UNSUPPORTED_EIP)
    }
    msg = 'Should throw for using paramByEIP() with an unsupported EIP'
    st.throws(f, /not supported$/, msg)

    f = function () {
      c.paramByEIP('notExistingTopic', 'Bls12381G1AddSmoke', 2537)
    }
    msg = 'Should throw for using paramByEIP() with a not existing topic'
    st.throws(f, /not defined$/, msg)

    msg = 'Should return Bls12381G1AddSmoke smoke price for EIP2537'
    st.equal(c.paramByEIP('smokePrices', 'Bls12381G1AddSmoke', 2537), 600, msg)
    st.end()
  })
