...
export class BlockHeader {
  ...
  public readonly smokeLimit: BN
  public readonly smokeUsed: BN
  ...
  public static fromHeaderData(headerData: HeaderData = {}, opts: BlockOptions = {}) {
    ...
    return new BlockHeader(
      ...
      smokeLimit ? new BN(toBuffer(smokeLimit)) : DEFAULT_GAS_LIMIT,
      smokeUsed ? new BN(toBuffer(smokeUsed)) : new BN(0),
  ...
  public static fromValuesArray(values: BlockHeaderBuffer, opts: BlockOptions) {
    ...
    return new BlockHeader(
      ...
      new BN(toBuffer(smokeLimit)),
      new BN(toBuffer(smokeUsed)),
  ...
  constructor(
    ...
    if (options.initWithGenesisHeader) {
      ...
      number = new BN(0)
      if (smokeLimit.eq(DEFAULT_GAS_LIMIT)) {
        smokeLimit = new BN(toBuffer(this._common.genesis().smokeLimit))
      }
      if (timestamp.isZero()) {
   ...
   * @param parentBlockHeader - the header from the parent `Block` of this header
   */
  validateSmokeLimit(parentBlockHeader: BlockHeader): boolean {
    const parentSmokeLimit = parentBlockHeader.smokeLimit
    const smokeLimit = this.smokeLimit
    const hardfork = this._getHardfork()

    const a = parentSmokeLimit.div(
      new BN(this._common.paramByHardfork('smokeConfig', 'smokeLimitBoundDivisor', hardfork))
    )
    const maxSmokeLimit = parentSmokeLimit.add(a)
    const minSmokeLimit = parentSmokeLimit.sub(a)

    return (
      smokeLimit.lt(maxSmokeLimit) &&
      smokeLimit.gt(minSmokeLimit) &&
      smokeLimit.gte(this._common.paramByHardfork('smokeConfig', 'minSmokeLimit', hardfork))
    )
  }
   ...
   *
   * @param blockchain - additionally validate against a @fourtwentyjs/blockchain
  ...
  async validate(blockchain?: Blockchain, height?: BN): Promise<void> {
    ...
    if (blockchain) {
      ...
      if (!this.validateSmokeLimit(header)) {
        throw new Error('invalid smoke limit')
      }
  ...
  raw(): BlockHeaderBuffer {
    return [
      ...
      unpadBuffer(toBuffer(this.smokeLimit)),
      unpadBuffer(toBuffer(this.smokeUsed)),
  ...
  toJSON(): JsonHeader {
    return {
      ...
      smokeLimit: bnToHex(this.smokeLimit),
      smokeUsed: bnToHex(this.smokeUsed),
