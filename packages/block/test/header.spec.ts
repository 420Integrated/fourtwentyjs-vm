...
import { Address, BN, zeros, KECCAK256_RLP, KECCAK256_RLP_ARRAY } from 'fourtwentyjs-util'
...
tape('[Block]: Header functions', function (t) {
  t.test('should create with default constructor', function (st) {
    function compareDefaultHeader(st: tape.Test, header: BlockHeader) {
      ...
      st.ok(header.smokeLimit.eq(new BN(Buffer.from('ffffffffffffff', 'hex'))))
      st.ok(header.smokeUsed.isZero())
  ...
  t.test('should test validateSmokeLimit', function (st) {
    const testData = require('./testdata/bcBlockSmokeLimitTest.json').tests
    const bcBlockSmokeLimitTestData = testData.BlockSmokeLimit2p63m1

    Object.keys(bcBlockSmokeLimitTestData).forEach((key) => {
      const genesisRlp = bcBlockSmokeLimitTestData[key].genesisRLP
      const parentBlock = Block.fromRLPSerializedBlock(genesisRlp)
      const blockRlp = bcBlockSmokeLimitTestData[key].blocks[0].rlp
      const block = Block.fromRLPSerializedBlock(blockRlp)
      st.equal(block.validateSmokeLimit(parentBlock), true)
    })
